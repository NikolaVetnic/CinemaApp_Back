...
public class DateTimeUtils {
    ...
    public static LocalDateTime dateFromString(String dateString) {
        ...
        if (!dateString.matches("(?<year>\\d{4})-(?<month>\\d{2})-(?<day>\\d{2})"))
            throw new IllegalArgumentException("Expected format is \"yyyy-MM-dd\", eg. \"2020-05-05\"");

        String dateTimeString = dateString + "T00:00";
